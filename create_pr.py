...
payload = {"title": "Fix for issue #10", "body": "Closes #10\n\nImplemented automated fix.", "head": "KartavyaDikshit:fix-issue-10", "base": "main"}
req = urllib.request.Request("https://api.github.com/repos/ibm-granite-community/mellea-cookbook/pulls", data=json.dumps(payload).encode(), headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json'}, method='POST')
try:
    with urllib.request.urlopen(req, context=ctx) as r: 
        print("[+] PR Created:", json.loads(r.read())['html_url'])
        os.chdir("..")
        shutil.rmtree("/Users/kartavyadikshit/Projects/Open Source/mellea-cookbook_bounty_parallel_19", ignore_errors=True)
